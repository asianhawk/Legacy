	...
	else
	{
		...
		if (strcmp(bitptr, DEMAND_BIT)==0)
		{
		   if (m_blnPrintAndApply==1)
			 ...
			 if (nTraceEnabled) LogEvents(TRACE_DEMAND_PAT_CCF_OFF);		
		   }
		   else
		   {
...
char *result,
	...
	if (ERROR_TRAP)
	{
		...
		csErrorChk = getPin(pin2);

		//printf("Pin[%d] Result = %s\n",pin2,csErrorChk);
		if(*csErrorChk==PRINTERERROR)
		{
			write (filedes, RESULT_NOT_OK, strlen (RESULT_NOT_OK));
			return 0;
		}
	}
