...
/* Added 051706 */
/* logic for motion controller */

#define CMD_MOTION_CONTROLLER "!MC"
...
static int
ProcessMC_Commands(int filedes, const char *csvfield_1, const char *csvfield_2, const char *csvfield_3, 
			    const char *csvfield_4, const char *csvfield_5)
{
char *result;
char buffer[100];
int  nTraceEnabled;
char strMC_Command[100];

char scanbuf[SCANNER_BUFFER_SIZE];
int mcs = TIOCM_RTS;
int lsr=0;
	memset(buffer,'\000',100);
	memset(strMC_Command,'\000',16);

	nTraceEnabled = Settings.nTraceEnabled;
	sprintf(buffer," ProcessMC_Commands  Start \narg1[%s] \narg2[%s] \narg3[%s] \narg4[%s] \narg5[%s]\n",
			csvfield_1, csvfield_2, csvfield_3, csvfield_4, csvfield_5); 
...
// 	"/dev/ttyS2"
	...
	Settings.mcontroller.fd =   openMCPort (Settings.mcontroller.dev, Settings.mcontroller.baud, (CS8 | CLOCAL | CREAD));


//	Settings.mcontroller.fd =   openMCPort ("/dev/ttyS2", Settings.mcontroller.baud, (CS8 | CLOCAL | CREAD));
	if (-1 == Settings.mcontroller.fd)
	{

		if (nTraceEnabled==1) LogEvents ("Error Opening IAI Controller");
		return -1;
	}	
	
//	strcpy(strMC_Command,"\x20n000000000082\x3");
	strcat(strMC_Command,"\002");
	strcat(strMC_Command,"0n000000000082");
	strcat(strMC_Command,"\003");
	sprintf(buffer,"ProcessMC_Commands = [%s]\000",strMC_Command);
	if (nTraceEnabled==1) LogEvents(buffer); 
	memset(buffer,'\000',100);

	// clear RTS
//	ioctl(Settings.mcontroller.fd,TIOCMBIC,&mcs);
	// set RTS
//	ioctl(Settings.mcontroller.fd,TIOCMBIC,&mcs); 
	...
	if (IsRTS())
	{
		if (nTraceEnabled==1) LogEvents("ProcessMC_Commands RTS Enabled"); 
		SetRTS(1);

	}
	else
	{
		if (nTraceEnabled==1) LogEvents("ProcessMC_Commands RTS NOT Enabled"); 

	}

	
	write(Settings.mcontroller.fd, strMC_Command, strlen(strMC_Command));

	int t = strtol("2", NULL, 10); 	/* Timeout */
	...
	if (IsRTS())
	{
		if (nTraceEnabled==1) LogEvents("ProcessMC_Commands RTS Enabled Should Be Off"); 
		

	}
	else
	{
		if (nTraceEnabled==1) LogEvents("ProcessMC_Commands RTS NOT Enabled Should Be Off"); 

	}

	if (1==inputTimeout(Settings.mcontroller.fd , 5))
	{
		if (nTraceEnabled==1) LogEvents("After InputTimeOut Before readMCLine"); 
		memset(scanbuf, '\000', SCANNER_BUFFER_SIZE);
		if (0==readMCLine(Settings.mcontroller.fd, scanbuf, SCANNER_BUFFER_SIZE))
		{
		...
		if (nTraceEnabled==1) LogEvents("After InputTimeOut After readMCLine"); 
	...
	do
	{
	   ioctl(Settings.mcontroller.fd,TIOCSERGETLSR,&lsr);	
	   memset(buffer, '\000', 100);
	   sprintf(buffer,"Waiting For Line Status Register To Clear lsr = [%d]",lsr);	
	   if (nTraceEnabled==1) LogEvents(buffer); 	
...
//	ioctl(Settings.mcontroller.fd,TIOCMBIC,&mcs);
	...
	close(Settings.mcontroller.fd);
	if (nTraceEnabled==1) LogEvents("Exit ProcessMC_Commands"); 
...
char buffer[100];
	...
	ioctl(Settings.mcontroller.fd,TIOCMGET,&nResult);	

	if (nTraceEnabled)
	{
	   memset(buffer, '\000', 100);
	   sprintf(buffer,"IsRTS()   TIOCMGET = [%d]",nResult);
	   if (nTraceEnabled==1) LogEvents(buffer); 	

	}
	if ((nResult & TIOCM_RTS)==TIOCM_RTS) return 1;
	return 0;
}
...
unsigned int nResult=0;
	...
	if (nMode==1)
	  ...
	  nResult &= ~TIOCM_RTS;	
	ioctl(Settings.mcontroller.fd,TIOCMSET,&nResult);
...
//		kill(getpid(),SIGINT);
	...
	else if (strcmp (command, CMD_MOTION_CONTROLLER) == 0)
	{
		ProcessMC_Commands (filedes, csvfield(1), csvfield(2), csvfield(3), csvfield(4), csvfield(5));
	}
